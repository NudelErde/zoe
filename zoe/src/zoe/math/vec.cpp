...
vec4 vec4::crossProduct(const vec4& b,const vec4& c) const {
	vec4 v;
	v.x=(this->y*b.z*c.w)+(this->w*b.y*c.z)+(this->z*b.w*c.y)-((this->w*b.z*c.y)+(this->z*b.y*c.w)+(this->y*b.w*c.z));
	v.y=(this->x*b.z*c.w)+(this->w*b.x*c.z)+(this->z*b.w*c.x)-((this->w*b.z*c.x)+(this->z*b.x*c.w)+(this->x*b.w*c.z));
	v.z=(this->x*b.y*c.w)+(this->w*b.x*c.y)+(this->y*b.w*c.x)-((this->w*b.y*c.x)+(this->y*b.x*c.w)+(this->x*b.w*c.y));
	v.w=(this->x*b.y*c.z)+(this->z*b.x*c.y)+(this->y*b.z*c.x)-((this->z*b.y*c.x)+(this->y*b.x*c.z)+(this->x*b.z*c.y));
	return v;
}
