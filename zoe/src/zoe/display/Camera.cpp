...
Camera2D::Camera2D(const vec2& position, const double& width, const double& height) : width(width), height(height) {
...
Camera3D::Camera3D(const vec3& pos, const vec3& rot, const double& fov, const double& ratio) : rotation(rot), fov(fov),
                                                                                             ratio(ratio) {
...
void Camera3D::setPitch(const double& pitch) {
    rotation.x = pitch;
    if (rotation.x > 2 * PI)
        rotation.x -= 2 * PI;
    updateCameraMatrix();
...
void Camera3D::setYaw(const double& yaw) {
    rotation.y = yaw;
    if (rotation.y > 2 * PI)
        rotation.y -= 2 * PI;
    updateCameraMatrix();
...
void Camera3D::setRoll(const double& roll) {
    rotation.z = roll;
    if (rotation.z > 2 * PI)
        rotation.z -= 2 * PI;
    updateCameraMatrix();
...
void Camera3D::setRotation(const double& pitch, const double& yaw, const double& roll) {
    rotation.x = pitch;
    rotation.y = yaw;
    rotation.z = roll;
    updateCameraMatrix();
...
void Camera3D::setDirection(const vec3& direction, const double& roll) {
...
void Camera3D::setFoV(const double& f) {
    fov = f;
    setProjectionMatrix(perspective(0.01f, 100, fov, ratio));
}

void Camera3D::setRatio(const double& rat) {
    ratio = rat;
    setProjectionMatrix(perspective(0.01f, 100, fov, ratio));
}
